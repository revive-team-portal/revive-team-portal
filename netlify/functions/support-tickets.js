...
exports.handler = async (event) => {
  ...
  let path = 'tickets?select=id,subject,status,priority,triage,ticket_type,source,reviewed,escalated,matched_order,snippet,courier_status,updated_at,created_at,gmail_thread_id,customer:customers(email,name,orders_count,lifetime_value,city,interactions(channel,direction,occurred_at)),messages(direction,from_addr,sent_at)&order=updated_at.desc&limit=300';
