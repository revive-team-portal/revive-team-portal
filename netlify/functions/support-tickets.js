...
exports.handler = async (event) => {
  ...
  let path = 'tickets?select=id,subject,status,priority,triage,reviewed,matched_order,snippet,updated_at,created_at,gmail_thread_id,customer:customers(email,name,interactions(channel,direction,occurred_at)),messages(direction,from_addr,sent_at)&order=updated_at.desc&limit=300';
