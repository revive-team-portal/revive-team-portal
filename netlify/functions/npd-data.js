  ...
  try {
    ...
    if (body.action === 'recipe_nips') {
      const [recs, ings, nuts, rates] = await Promise.all([
        db('recipe?select=id,sku,flavour,version_label,g_per_waffle,moisture_pct&active=eq.true&is_current=eq.true&order=sku'),
      ...
      return json(200, { recipes: out, serves_per_pack: serves, wopples_per_serving: wops });
    }
