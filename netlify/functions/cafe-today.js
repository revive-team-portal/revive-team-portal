...
exports.handler = async (event) => {
  ...
  try {
    ...
    const [rows, oc, ss] = await Promise.all([db('pos_today?id=eq.1&select=sales,covers,updated_at'), orderCounts(), shopifySums()]);
    const t = (rows && rows[0]) || {};
    return { statusCode: 200, headers: { 'Content-Type': 'application/json', 'Cache-Control': 'no-store', 'Access-Control-Allow-Origin': '*' },
      body: JSON.stringify({ sales: t.sales, covers: t.covers, updated_at: t.updated_at,
        ...
        orders_to_fulfil: oc.orders_to_fulfil, orders_fulfilled_today: oc.orders_fulfilled_today }) };
