// Sends a reply from the cafe@ support mailbox, threaded, and records it on the ticket.
...
const FOOTER = 'https://revivealicious.com/cdn/shop/files/2x1_Banner_2026_75600d3d-3499-4602-af5f-77a29db1fd87.jpg?width=1000';
...
exports.handler = async (event) => {
  ...
  try {
    // fetch the ticket to get gmail thread id + subject
    ...
    const html = '<div style="font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#222;white-space:pre-wrap">'+esc(text)+'</div>'
      + '<br><a href="https://revive.co.nz"><img src="'+FOOTER+'" alt="Revive" style="display:block;max-width:600px;width:100%;height:auto;border:0"></a>';
    ...
    // record the outbound message + bump ticket
    await rest('messages', { method:'POST', headers:{ Prefer:'resolution=merge-duplicates,return=minimal' }, body: JSON.stringify({
      gmail_message_id: d.id, ticket_id: body.id, direction:'outbound', from_addr: from, to_addr: to,
