...
function serviceType(order, t){
  const hay=((order.tags||[]).join(' ')+' '+lineNodes(order).map(li=>li.title||'').join(' ')+' '+productTypes(order)).toLowerCase();
  if(/perishable|chilled|frozen|cold|fresh|refriger|heat.{0,3}eat|\bmeal/.test(hay)) return 'Perishable';
...
async function runScan(opts={}){
  ...
  while(pages<8){
    ...
    for(const e of edges){
      ...
      if(!tn) continue;
      // Skip in-store pickups and anything that isn't a real courier tracking number.
      if(!/\d/.test(tn) || !/^[A-Za-z0-9-]{8,}$/.test(tn) || /pick.?up|in.?store|instore|counter|collect/i.test(tn)) continue;
      tracked++;
      ...
      const status = (rawStatus && rawStatus!=='Unknown' && rawStatus!=='lookup failed') ? rawStatus : (latestEvent || rawStatus);
      const hay=(rawStatus+' '+eventsText);
      const delivered = /deliver(ed)?/i.test(rawStatus) || /\bdelivered\b/i.test(eventsText);
      let reason='', detail='';
      if(!delivered){
      ...
      const value=Number((o.totalPriceSet&&o.totalPriceSet.shopMoney&&o.totalPriceSet.shopMoney.amount)||(o.currentTotalPriceSet&&o.currentTotalPriceSet.shopMoney&&o.currentTotalPriceSet.shopMoney.amount))||null;
