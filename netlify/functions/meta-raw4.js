...
const ACCT=process.env.META_AD_ACCOUNT||'act_242089740673955';const GUARD='3988e8819f54f36c6c886dd4b564a1cf';
exports.handler=async(e)=>{const q=(e&&e.queryStringParameters)||{};
 ...
 let url=GRAPH+'/'+p+'?'+String(q.q||'')+'&limit='+(q.limit||'500')+'&access_token='+encodeURIComponent(TOKEN);
 const all=[];let meta=null;
 try{for(let i=0;i<25&&url;i++){const r=await fetch(url);const j=await r.json().catch(()=>({}));
