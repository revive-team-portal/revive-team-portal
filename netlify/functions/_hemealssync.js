...
async function syncHeatEat(start, end) {
  ...
  if (op.url) {
    ...
    for (const ln of lines) { let o; try { o = JSON.parse(ln); } catch { continue; } if (o.__parentId && o.product && MEAL_TYPES.has(o.product.productType || '')) { const we = orderWeek[o.__parentId]; if (we) wk[we] = (wk[we] || 0) + (Number(o.quantity) || 0) * mealsPerUnit(o.product.title); } }
  }
  const weekRows = await appsDb('week?select=period_end');
  const exist = new Set((weekRows || []).map(x => x.period_end));
  const today = new Date().toISOString().slice(0, 10);
  const ov = await appsDb("fact?select=period_end,metric_code&period_type=eq.week&is_override=eq.true&metric_code=in.(heat_eat_sold)");
