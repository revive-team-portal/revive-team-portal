// TEMPORARY read-only per-order attribution. Delete after use. ?k=<key>&start=YYYY-MM-DD&end=YYYY-MM-DD (NZ)
...
exports.handler = async (event) => {
  ...
  const Q = 'query($q:String!,$after:String){ orders(first:100, query:$q, after:$after, sortKey:CREATED_AT){ pageInfo{ hasNextPage endCursor } nodes{ ' + F + ' } } }';
  try {
