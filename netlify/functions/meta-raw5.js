...
const ACCT=process.env.META_AD_ACCOUNT||'act_242089740673955';const GUARD='54a741bf7d942b990b6e3e9628ac599e';
exports.handler=async(e)=>{const q=(e&&e.queryStringParameters)||{};
 ...
 let url=GRAPH+'/'+p+'?'+String(q.q||'')+'&limit='+(q.limit||'200')+'&access_token='+encodeURIComponent(TOKEN);
 const all=[];let meta=null;
 try{for(let i=0;i<12&&url;i++){const r=await fetch(url);const j=await r.json().catch(()=>({}));
