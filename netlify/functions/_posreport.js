...
function parseReport(tsv) {
  ...
  const depts = [], media = []; let covers = 0, txns = 0;
  ...
  return { depts, deptTotal: depts.reduce((s, d) => s + d.amount, 0), deptQty: depts.reduce((s, d) => s + d.qty, 0), covers, txns, media, mediaTotal: media.reduce((s, d) => s + d.amount, 0) };
}
